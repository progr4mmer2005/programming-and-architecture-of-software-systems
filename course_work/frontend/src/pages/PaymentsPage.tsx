...
export default function PaymentsPage() {
  ...
  const openCreate = () => { setEditing(null); setForm(initialState); setError(''); setIsModalOpen(true); };
  const openEdit = (payment: Payment) => { setEditing(payment); setForm({ contract: String(payment.contract), stage: payment.stage ? String(payment.stage) : '', act: payment.act ? String(payment.act) : '', type: payment.type, amount: String(payment.amount || ''), planned_date: payment.planned_date || '', paid_date: payment.paid_date || payment.actual_date || '', status: payment.status, description: payment.description || '' }); setError(''); setIsModalOpen(true); };
  const closeModal = () => { setEditing(null); setForm(initialState); setError(''); setIsModalOpen(false); };
  ...
  return <div className="space-y-6"><PageIntro eyebrow="Платежи" title="Плановые и фактические оплаты" actions={canManagePayments ? <Button onClick={openCreate}><Plus className="h-4 w-4" />Новый платёж</Button> : undefined} />
    ...
    <Modal open={isModalOpen} onClose={closeModal} title={editing ? 'Редактирование платежа' : 'Новый платёж'} size="lg"><div className="space-y-5">{error ? <div className="rounded-2xl border border-[rgba(180,79,64,0.18)] bg-[rgba(180,79,64,0.08)] px-4 py-3 text-sm text-[var(--danger)]">{error}</div> : null}<div className="grid gap-4 md:grid-cols-2"><Field label="Договор"><Select value={form.contract} onChange={(event) => setForm((current) => ({ ...current, contract: event.target.value, stage: '', act: '' }))}><option value="">Выберите договор</option>{contracts.map((contract) => <option key={contract.id} value={contract.id}>{contract.title}</option>)}</Select></Field><Field label="Тип"><Select value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as PaymentType }))}>{paymentTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></Field><Field label="Статус"><Select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value as PaymentStatus }))}>{paymentStatusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</Select></Field><Field label="Сумма"><Input type="number" value={form.amount} onChange={(event) => setForm((current) => ({ ...current, amount: event.target.value }))} /></Field><Field label="Этап"><Select value={form.stage} onChange={(event) => setForm((current) => ({ ...current, stage: event.target.value }))}><option value="">Без этапа</option>{stages.map((stage) => <option key={stage.id} value={stage.id}>{stage.name}</option>)}</Select></Field><Field label="Акт"><Select value={form.act} onChange={(event) => setForm((current) => ({ ...current, act: event.target.value }))}><option value="">Без акта</option>{acts.map((act) => <option key={act.id} value={act.id}>{act.number} - {act.title}</option>)}</Select></Field><Field label="Плановая дата"><Input type="date" value={form.planned_date} onChange={(event) => setForm((current) => ({ ...current, planned_date: event.target.value }))} /></Field><Field label="Дата оплаты"><Input type="date" value={form.paid_date} onChange={(event) => setForm((current) => ({ ...current, paid_date: event.target.value }))} /></Field><div className="md:col-span-2"><Field label="Описание"><Input value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} /></Field></div></div><div className="flex justify-end gap-3"><Button variant="secondary" onClick={closeModal}>Отмена</Button><Button onClick={() => saveMutation.mutate()} busy={saveMutation.isPending}>Сохранить</Button></div></div></Modal>
  </div>;
}
